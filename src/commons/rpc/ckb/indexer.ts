...
export class CkbIndexer implements Indexer {
  ...
  async waitForSync(blockDifference = 0): Promise<void> {
    ...
    let indexerTipNumber = parseInt((await this.tip()).block_number, 16);
    while (indexerTipNumber + blockDifference < rpcTipNumber) {
      await asyncSleep(1000);
      indexerTipNumber = parseInt((await this.tip()).block_number, 16);
    }
  }
