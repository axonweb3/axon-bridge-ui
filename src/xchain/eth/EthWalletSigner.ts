...
export class EthWalletSigner extends AbstractWalletSigner<EthereumNetwork> {
  ...
  async _sendToNervos(
    raw: NervosNetwork["RawTransaction"]
  ): Promise<{ txId: string }> {
    ...
    const paramsToSign = window.ethereum.isSafePal
      ? [messageToSign]
      : [this.identityXChain(), messageToSign];
    ...
    skeleton = skeleton.update("witnesses", (witnesses) =>
      witnesses.push(signedWitness)
    );

    const signedTx = helpers.createTransactionFromSkeleton(skeleton);
    const txHash = await new RPC(
      process.env.REACT_APP_CKB_RPC_URL
    ).send_transaction(signedTx, "passthrough");
    return { txId: txHash };
  }

  async _sendToXChain(
    raw: EthereumNetwork["RawTransaction"]
  ): Promise<{ txId: string }> {
  ...
  async approve(
    asset: EthereumNetwork["DerivedAssetIdent"]
  ): Promise<{ txId: string }> {
