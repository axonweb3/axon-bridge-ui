...
export const WalletConnectorButton: React.FC<WalletConnectorButtonProps> = (
  ...
  const { signer, walletConnectStatus, wallet, direction } =
  ...
  const buttonContent = useMemo(() => {
    if (walletConnectStatus === ConnectStatus.Disconnected)
      return disconnectedContent;
    if (walletConnectStatus === ConnectStatus.Connecting)
      return connectingContent;
    if (!signer) return connectingContent;

    if (userIdentityMode === "alwaysXChain")
      return <UserIdent ident={signer.identityXChain()} />;
    if (userIdentityMode === "alwaysNervos")
  ...
  const connecting =
    walletConnectStatus === ConnectStatus.Connecting ||
    (walletConnectStatus === ConnectStatus.Connected && !signer);
