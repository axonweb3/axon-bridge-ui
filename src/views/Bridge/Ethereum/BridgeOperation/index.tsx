...
import {
  useBridgeFeeQuery,
  useValidateBridgeOperationForm,
  ValidateResult,
} from "hooks/bridge-operation";
...
const Help: React.FC<{ validateStatus: "error" | ""; help?: string }> = ({
  validateStatus,
  help,
}) => {
...
export const BridgeOperationForm: React.FC = () => {
  ...
  const { signer, direction, switchBridgeDirection } =
  ...
  const formik = useFormik<ValidateResult>({
    ...
    validate,
    initialTouched: {
      bridgeInInputAmount: !!initAmount,
  ...
  const { mutateAsync: sendBridgeTransaction, isLoading: isBridgeLoading } =
    useSendBridgeTransaction();
  const { mutateAsync: sendApproveTransaction, isLoading: isApproveLoading } =
  ...
  function onSubmit() {
    ...
    if (!selectedAsset || (!recipient && !needApprove) || !selectedAsset.shadow)
      return;

    if (needApprove) {
      sendApproveTransaction({
        asset: selectedAsset,
        addApprove: allowance.addApprove,
      }).then(resetForm);
    } else {
      const asset =
        direction === BridgeDirection.In
          ? selectedAsset.copy()
          : selectedAsset.shadow?.copy();
      if (asset.info?.decimals == null) boom("asset info is not loaded");

      asset.amount = BeautyAmount.fromHumanize(
        bridgeFromAmount,
        asset.info.decimals
      ).val.toString();
  ...
  return (
    <BridgeViewWrapper>
          ...
          extra={
            selectedAsset && (
              <Button
                ...
                onClick={() =>
                  setBridgeFromAmount(
                    BeautyAmount.from(selectedAsset).humanize({
                      separator: false,
                    })
                  )
                }
      ...
      <Row justify="center" align="middle">
        <Icon
          style={{ fontSize: "24px" }}
          component={BridgeDirectionIcon}
          onClick={() => switchBridgeDirection()}
        />
      </Row>
      ...
      <div className="input-wrapper">
        <UserInput
          ...
          placeholder={
            direction === BridgeDirection.In
              ? "input ckb address"
              : `input ${network.toLowerCase()} address`
          }
          value={recipient}
