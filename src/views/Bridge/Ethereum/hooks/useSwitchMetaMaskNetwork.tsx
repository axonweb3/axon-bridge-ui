...
export function useSwitchMetaMaskNetwork(): UseMutationResult<
  void,
  unknown,
  SwitchInputValues
> {
    ...
    async (input: SwitchInputValues) => {
      try {
        ...
        await provider.send("wallet_addEthereumChain", [
          {
            chainId: `0x${Number(
              process.env.REACT_APP_AXON_ENABLE_CHAIN_ID
            ).toString(16)}`,
            ...
            blockExplorerUrls: [
              process.env.REACT_APP_TX_EXPLORER_BSC.slice(0, -4),
            ],
          },
        ]);
      } catch (switchError) {
        ...
        if (switchError.code === 4902) {
          if (network === "Bsc") {
            await provider.send("wallet_addEthereumChain", [
              {
                chainId: `0x${Number(
                  process.env.REACT_APP_BSC_ENABLE_CHAIN_ID
                ).toString(16)}`,
    ...
    },
    {
      onError(error) {
        const errorMsg: string = utils.hasProp(error, "message")
          ...
          : "Unknown error";
        Modal.error({
          title: "Switch MetaMask Network failed",
          content: errorMsg,
          width: 360,
        });
      },
