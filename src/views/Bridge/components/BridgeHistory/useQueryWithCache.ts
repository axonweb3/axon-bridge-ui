...
export function useQueryWithCache(
...
): TransactionSummaryWithStatus[] | null | undefined {
  const { signer, direction, nervosModule, api, network } =
  ...
  const filter = useMemo<
    API.GetBridgeTransactionSummariesPayload | undefined
  >(() => {
    if (!asset || !signer) return undefined;
    const userNetwork =
      direction === BridgeDirection.In ? ethereumNetwork : nervosModule.network;
    const userIdent =
      direction === BridgeDirection.In
        ? signer.identityXChain()
        : signer.identityNervos();
    return {
  ...
  const { transactions: cachedTransactions, removeTransactions } =
  ...
  const cachedTransactionsOfSender = cachedTransactions.filter(
    (item) =>
      ...
      ((item.txSummary.fromAsset.ident === asset.ident &&
        item.txSummary.toAsset.ident === asset.shadow?.ident) ||
        (item.txSummary.toAsset.ident === asset.ident &&
          item.txSummary.fromAsset.ident === asset.shadow?.ident))
  );

  if (!query.data)
    return cachedTransactionsOfSender as
      | TransactionSummaryWithStatus[]
      | null
      | undefined;
  const backendCoveredTransactions: string[] = [];
  const availableCachedTransactions = cachedTransactionsOfSender.filter(
    (cachedItem) => {
      if (
        query.data.find(
          (item) =>
            item.txSummary.fromTransaction.txId ===
            cachedItem.txSummary.fromTransaction.txId
        )
      ) {
        backendCoveredTransactions.push(
          cachedItem.txSummary.fromTransaction.txId
        );
        return false;
      }
  ...
  return query.data
    ...
    .sort(
      (s1, s2) =>
        +new Date(
          s2.txSummary.toTransaction?.timestamp ||
            s2.txSummary.fromTransaction.timestamp
        ) -
        +new Date(
          s1.txSummary.toTransaction?.timestamp ||
            s1.txSummary.fromTransaction.timestamp
        )
    );
}
