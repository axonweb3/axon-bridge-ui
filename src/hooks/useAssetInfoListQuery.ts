...
interface UseAssetInfoState {
  query: QueryObserverResult<{
    xchain: Record<string, Asset>;
    nervos: Record<string, nervos.SUDT>;
  }>;
...
export function useAssetInfoListQuery(): UseAssetInfoState {
  ...
  const query = useQuery(["getAssetAssetWithInfo", { network }], async () => {
    ...
    const xchainAssetsInfo = infoList
      .filter(X.isCurrentNetworkAsset)
      .map<Asset>((assetWithInfo) => {
        ...
        return boom(
          `Unknown network asset with ${JSON.stringify(assetWithInfo)}`
        );
      });

    // xchain asset.shadow
    const nervosAssetsInfo = xchainAssetsInfo.map<nervos.SUDT>(
      (xchainAsset) => {
  ...
  const infoOf = useCallback(
    (asset: { network: string; ident: string }) => {
      ...
      if (asset.network === NERVOS_NETWORK)
        return query.data.nervos[asset.ident]?.info;
