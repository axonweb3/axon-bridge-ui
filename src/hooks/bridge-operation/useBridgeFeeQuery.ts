...
export function useBridgeFeeQuery(): QueryObserverResult<
  API.GetBridgeInNervosBridgeFeeResponse,
  Error
> {
  ...
  const { asset, bridgeFromAmount: input } =
  ...
  const amount = useMemo(() => {
    if (
      ...
      !asset.info ||
      !bridgeInAmount ||
      !(Number(bridgeInAmount) > 0)
    )
      return "";

    return BeautyAmount.fromHumanize(
      bridgeInAmount,
      asset.info.decimals
    ).val.toString();
  ...
  return useQuery(
    ...
    () => {
      ...
      if (direction === BridgeDirection.In) {
        return api.getBridgeInNervosBridgeFee({
